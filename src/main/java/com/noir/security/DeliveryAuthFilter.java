...
public class DeliveryAuthFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        // Login is public; everything else under /api/delivery/ is protected
        if (path.startsWith("/api/delivery/") && !path.equals("/api/delivery/login")) {
            String header = request.getHeader("Authorization");
            String token = (header != null && header.startsWith("Bearer "))
                    ? header.substring(7) : null;
            try {
                StaffUser staff = staffAuthService.verifyDeliveryToken(token);
                request.setAttribute("deliveryStaff", staff);
